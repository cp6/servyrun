                ...
                <div className="grid gap-2 grid-cols-1 sm:grid-cols-4 sm:gap-4">
                    {
                        servers.length === 0
                            ?
                            <ResourceEmptyText resource={'servers'}></ResourceEmptyText>
                            :
                            servers.map(server =>
                                <section key={server.id}
                                    ...
                                    <div className="md:py-2 py-4 px-2 mx-auto max-w-6xl">
                                            ...
                                            <div>            {server.operating_system !== null ?
                                                    ...
                                                    className="bg-indigo-200 text-indigo-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-indigo-900 dark:text-indigo-300">{server.type.name}</span>
                                                {
                                                    (() => {
                                                        if (server.conn === null) {
                                                            return (<span
                                                                className="bg-red-100 text-red-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-red-900 dark:text-red-300">No connection</span>)
                                                        }
                                                    })()
                                                }
                                            </div>
                                        ...
                                        <div className="flex flex-col items-center pb-3">
                                            ...
                                            <span
                                                className="text-sm text-gray-500 dark:text-gray-400">{server.title}</span>
                                        </div>
                                    </div>
                                </section>
                            )
