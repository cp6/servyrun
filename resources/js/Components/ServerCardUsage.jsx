...
export default function ServerCardUsage({serverId, resource}) {
    ...
    const refreshUsage= () => {
        ...
        axios.get(route('server.usage', serverId)).then(response => {
            setCpu(response.data.cpu_used_percent);
            setRam(response.data.ram_used_percent);
            setDisk(response.data.disk_available);
        ...
        <dl className="flex items-center space-x-6 mt-2">
            <div>
                <dt className="mb-2 font-light leading-none text-gray-900 dark:text-gray-300"><HiRefresh
                    title={'Refresh usage stats'} onClick={refreshUsage} className='mt-2 h-5 w-5 hover:cursor-pointer'/></dt>
                <dd className="mb-4 font-semibold text-gray-500 sm:mb-5 dark:text-white"></dd>
            </div>
            <div>
                <dt className="mb-2 font-light leading-none text-gray-900 dark:text-gray-300">CPU %</dt>
                <dd className="mb-4 font-semibold text-gray-500 sm:mb-5 dark:text-white">{cpu}</dd>
            </div>
            <div>
                <dt className="mb-2 font-light leading-none text-gray-900 dark:text-gray-300">RAM %</dt>
                <dd className="mb-4 font-semibold text-gray-500 sm:mb-5 dark:text-white">{ram}</dd>
            </div>
            <div>
                <dt className="mb-2 font-light leading-none text-gray-900 dark:text-gray-300">DISK %</dt>
                <dd className="mb-4 font-semibold text-gray-500 sm:mb-5 dark:text-white">{disk}</dd>
            </div>
            <div>
                <dt className="mb-2 font-light leading-none text-gray-900 dark:text-gray-300">DISK Avail</dt>
                <dd className="mb-4 font-semibold text-gray-500 sm:mb-5 dark:text-white">{diskAvailable} GB</dd>
            </div>
        </dl>
    );
}
