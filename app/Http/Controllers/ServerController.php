...
class ServerController extends Controller
{
    ...
    public function show(Server $server)
    {
        ...
        return Inertia::render('Servers/Show', [
            'resource' => $server->where('id', $server->id)->with(['type', 'location', 'ips', 'ip_ssh', 'conn'])->first(),
            'hasAlert' => \Session::exists('alert_type'),
            'alert_type' => \Session::get('alert_type'),
            'alert_message' => \Session::get('alert_message')
        ]);
    ...
    public function ip(Server $server): \Illuminate\Http\JsonResponse
    {
        ...
        return response()->json(array('ip' => $data->ip_ssh->ip));
    }
    ...
    {//Gets IP from A record for a domain
        switch ($type) {
            case "A":
                ...
                if (isset($data['0']['ip'])) {
                    return response()->json(array('ip' => $data['0']['ip']));
                }
                break;
            case "AAAA":
                $data = dns_get_record($domain_name, DNS_AAAA);
                if (isset($data['0']['ipv6'])) {
                    return response()->json(array('ip' => $data['0']['ipv6']));
                }
                break;
        }
        return response()->json(array('ip' => null));
    }
